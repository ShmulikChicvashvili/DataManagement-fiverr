...
public class MultiTableTDatabaseTest {
	...
	public void testRegisterToActivity()
		...
		addUsers(users, user2service, user2task, 4);

		DBUser u0 = users.get(0);
		DBUser u1 = users.get(1);
		DBUser u2 = users.get(2);
		DBUser u3 = users.get(3);

		DBPaidService s = new DBPaidService("s1", u0.getUsername(), 2, 3, 0);
		multiDB.addPaidService(s);
		...
		int maxServices = 6;
		int maxTasks = 4;
		fillUsersActivities(users, user2service, user2task, maxServices,
				maxTasks);
		DBPaidService s1 = new DBPaidService(u0.getUsername() + "_s8",
				u0.getUsername(), 1, 1, 0);
		DBPaidService s2 = new DBPaidService(u0.getUsername() + "_s9",
				u0.getUsername(), 2, 1, 0);
		...
		for (DBUser u : users) {
			for (DBPaidService service : user2service.get(u.getUsername())) {
				assertFalse(multiDB.registerToActivity(service.getId(),
						u.getUsername()));
			}
			for (DBPaidTask task : user2task.get(u.getUsername())) {
				assertFalse(multiDB.registerToActivity(task.getId(),
						u.getUsername()));
			}
		}
		...
		for (DBPaidService service : user2service.get(u0.getUsername())) {
			registerToService(multiDB, service, u0, u1);
		}
		for (DBPaidTask task : user2task.get(u0.getUsername())) {
			registerToTask(multiDB, task, u0, u1);
		}
		...
		registerToService(multiDB, s2, u0, u2);
		assertEquals(u0, usersDB.getUser(u0.getUsername()));
		...
		assertFalse(multiDB.registerToActivity(
				...
				u2.getUsername()));
		assertFalse(multiDB.registerToActivity(user2task.get(u1.getUsername())
				.get(0).getId(), u2.getUsername()));
	...
	public void testUnregisterFromActivity()
		...
		DBPaidService s2 = new DBPaidService(u0.getUsername() + "_s9",
				u0.getUsername(), 2, 1, 0);
		DBPaidTask t1 = new DBPaidTask(u0.getUsername() + "_t8",
				u0.getUsername(), 1, 1, 0);
		...
		addUserActivitiesToDB(users, user2service, user2task);
		assertFalse(multiDB.unregisterFromActivity(s1.getId(),
				"username_not_exist"));
		...
		for (DBPaidService s : user2service.get(u0.getUsername())) {
			registerToService(multiDB, s, u0, u1);
		}
		registerToTask(multiDB, t1, u0, u1);
		assertUsersEqual(users, user2service, user2task);
		...
		unregisterFromService(multiDB, s2, u0, u2);
		assertUsersEqual(users, user2service, user2task);
		...
		assertFalse(multiDB.registerToActivity(s1.getId(), u2.getUsername()));
		unregisterFromService(multiDB, s1, u0, u1);
		registerToService(multiDB, s1, u0, u2);

		// check u2 can register after u1 has unregistered from t1
		assertFalse(multiDB.registerToActivity(t1.getId(), u2.getUsername()));
		unregisterFromTask(multiDB, t1, u0, u1);
		registerToTask(multiDB, t1, u0, u2);

		// check u1 can unregister from s2
		unregisterFromService(multiDB, s2, u0, u1);
	...
	public void testDeleteUser()
		...
		DBUser u2 = users.get(2);
		DBUser u3 = users.get(3);
		DBPaidService u0s1 = new DBPaidService(u0.getUsername() + "_s8",
				u0.getUsername(), 1, 1, 0);
		DBPaidService u0s2 = new DBPaidService(u0.getUsername() + "_s9",
				u0.getUsername(), 2, 1, 0);
		DBPaidTask u0t1 = new DBPaidTask(u0.getUsername() + "_t8",
				u0.getUsername(), 1, 1, 0);
		DBPaidTask u1t1 = new DBPaidTask(u1.getUsername() + "_t8",
				u1.getUsername(), 3, 1, 0);

		// check a simple empty user deletion works
		deleteUser(users, user2service, user2task, u0);
		addUser(users, u0);
		...
		checkOfferedActivities(user2service, user2task, u0, u1, u2);
		checkOfferedActivities(user2service, user2task, u1, u0, u2);
		checkOfferedActivities(user2service, user2task, u2, u0, u1);

		registerToService(multiDB, u0s1, u0, u1);
		registerToService(multiDB, u0s2, u0, u1);
		registerToService(multiDB, u0s2, u0, u2);
		registerToTask(multiDB, u0t1, u0, u2);
		registerToTask(multiDB, u1t1, u1, u0);
		registerToTask(multiDB, u1t1, u1, u2);
		...
		assertUsersEqual(users, user2service, user2task);
		checkOfferedActivities(user2service, user2task, u1, u2);
		checkOfferedActivities(user2service, user2task, u2, u1);
	...
	private void checkOfferedActivities(
			Map<String, List<DBPaidService>> user2service,
			Map<String, List<DBPaidTask>> user2task, DBUser user, DBUser friend)
		...
		assertEquals(user2service.get(friend.getUsername()),
				activitiesDB.getServicesOfferedToUser(user.getUsername(), 0,
						100));
		assertEquals(user2task.get(friend.getUsername()),
				activitiesDB.getTasksOfferedToUser(user.getUsername(), 0, 100));
	}

	private void checkOfferedActivities(
			Map<String, List<DBPaidService>> user2service,
			Map<String, List<DBPaidTask>> user2task, DBUser user,
			DBUser friend1, DBUser friend2)
			throws com.servicebook.database.exceptions.paidActivities.InvalidParameterException,
			DatabaseUnkownFailureException, FriendshipsTableNotExist {
		assertEquals(
				concatLists(user2service.get(friend1.getUsername()),
						user2service.get(friend2.getUsername())),
				activitiesDB.getServicesOfferedToUser(user.getUsername(), 0,
						100));
		assertEquals(
				concatLists(user2task.get(friend1.getUsername()),
	...
	private void deleteUser(List<DBUser> users,
			Map<String, List<DBPaidService>> user2service,
			Map<String, List<DBPaidTask>> user2task, DBUser u)
		...
		assertEquals(u, usersDB.getUser(u.getUsername()));
		assertEquals(user2service.get(u.getUsername()),
				activitiesDB.getServicesOfferedByUser(u.getUsername(), 0, 100));
		assertEquals(user2task.get(u.getUsername()),
	...
	private int randInt(int min, int max) {
		int $ = rand.nextInt((max - min) + 1) + min;
		return $;
	}

	private void addUsers(List<DBUser> users,
			Map<String, List<DBPaidService>> user2service,
			Map<String, List<DBPaidTask>> user2task, int numUsers)
			throws ElementAlreadyExistsException,
			DatabaseUnkownFailureException, InvalidParamsException {
	...
	private void assertUsersEqual(List<DBUser> users,
			Map<String, List<DBPaidService>> user2service,
		...
		for (DBUser u : users) {
			assertEquals(u, usersDB.getUser(u.getUsername()));
			assertEquals(user2service.get(u.getUsername()),
					activitiesDB.getServicesOfferedByUser(u.getUsername(), 0,
							100));
			assertEquals(user2task.get(u.getUsername()),
					activitiesDB.getTasksOfferedByUser(u.getUsername(), 0, 100));
		}
	...
	private void addUserActivitiesToDB(List<DBUser> users,
			Map<String, List<DBPaidService>> user2service,
			Map<String, List<DBPaidTask>> user2task)
	...
	private void fillUsersActivities(List<DBUser> users,
			Map<String, List<DBPaidService>> user2service,
			Map<String, List<DBPaidTask>> user2task, int maxServices,
			int maxTasks) {
		int count = 0;
		for (DBUser u : users) {
			count++;
	...
	private void registerToService(MultiTableDatabaseImpl multiDB,
			DBPaidService s, DBUser creator, DBUser registering)
			throws DatabaseUnkownFailureException, InvalidParameterException {
		assertTrue(multiDB.registerToActivity(s.getId(),
	...
	private void unregisterFromService(MultiTableDatabaseImpl multiDB,
			DBPaidService s, DBUser creator, DBUser registering)
			throws DatabaseUnkownFailureException, InvalidParameterException {
		assertTrue(multiDB.unregisterFromActivity(s.getId(),
				registering.getUsername()));
		assertEquals(s.getUsername(), creator.getUsername());
		s.setNumRegistered((short) (s.getNumRegistered() - 1));
		creator.addBalance(-1);
	...
	private void registerToTask(MultiTableDatabaseImpl multiDB, DBPaidTask t,
			DBUser creator, DBUser registering)
			throws DatabaseUnkownFailureException, InvalidParameterException {
		assertTrue(multiDB.registerToActivity(t.getId(),
				registering.getUsername()));
	...
	private void unregisterFromTask(MultiTableDatabaseImpl multiDB,
			DBPaidTask t, DBUser creator, DBUser registering)
			throws DatabaseUnkownFailureException, InvalidParameterException {
		assertTrue(multiDB.unregisterFromActivity(t.getId(),
				registering.getUsername()));
		assertEquals(t.getUsername(), creator.getUsername());
		t.setNumRegistered((short) (t.getNumRegistered() - 1));
		creator.addBalance(1);
